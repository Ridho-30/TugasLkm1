...
namespace TugasLkm1.Controllers
{
    ...
    public class CustomersController : ControllerBase
    {
        ...
        public CustomersController(CustomerRepository repo) { _repo = repo; }
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                ...
                if (data is null)
                    return NotFound(new { status = "error", message = "Customer tidak ditemukan" });
                return Ok(new { status = "success", data });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                if (!result)
                    return NotFound(new { status = "error", message = "Customer tidak ditemukan" });
                return Ok(new { status = "success", message = "Customer berhasil dihapus" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { status = "error", message = ex.Message });
